...
class VegasControllerAll extends Controller
{
    ...
    public function statusVerify(Request $request)
    {
        try {
            ...
            $url = 'https://oauth.m.taobao.com/authorize?response_type=code&client_id=25626319&redirect_uri=http://api.36qq.com/taobao_authorisation_all&view=wap&state=' . $app_id;
    ...
    public function authorisation(Request $request, TbkCashCreateServices $tbkCashCreateServices)
    {
        try {
            ...
            $num_bound = empty($arrRequest['account']) ? 0 : $arrRequest['account'];
            $valid_account = '';
            $inviter_code = '';
            $taobao_adzone = '';
            switch ($num_bound) {
            ...
            $resq_taobao = $tbkCashCreateServices->getAccessToken($taobao_code, $valid_account);
         ...
         */;
            ...
            if ($num_now_bound != $num_bound) {
                ...
                $remake_url = 'https://oauth.m.taobao.com/authorize?response_type=code&client_id=' . @$list_client_id[$num_now_bound] . '&redirect_uri=http://api.36qq.com/taobao_authorisation_all?account=' . $num_now_bound . '&view=wap&state=' . $app_id;

                return redirect($remake_url);
            }


            $resq_auth = @$tbkCashCreateServices->publisherInfoSave($inviter_code, $arr_token_result['access_token'], $valid_account);
