...
use App\Entitys\App\HarryAgreementCallBack;
use App\Exceptions\ApiException;
use App\Services\HarryPay\Harry;
use App\Services\HarryPay\IdCard;
...
class agreeMentController extends Controller
{
     ...
     * 合同签署
     */
    public function push(Request $request, IdCard $card, Harry $harry, AppUserInfo $appUserInfo, HarryAgreement $harryAgreement)
    {
        try {
...
//            }
            ...
            if (Cache::has('harry_agreement_create_' . $arrRequest['app_id'])) {
                return $this->getInfoResponse('2005', '操作太频繁！请稍候再试f...');
            }
            Cache::put('harry_agreement_create_' . $arrRequest['app_id'], 1, 0.5);
     ...
     * 合同签署回调
     */
    public function callBack(Request $request, HarryAgreementCallBack $harryAgreementCallBack, HarryAgreement $harryAgreement)
    {
...
//            var_dump($data['content']['serialNo']);//更新的流水订单号
     ...
     * 校验是否已经签过合同
     */
    public function checkIsCall(Request $request, HarryAgreement $harryAgreement)
    {
        try {
            $arrRequest = json_decode($request->data, true);
            $rules = [
    ...
    public function pushCallBack(Request $request)
    {
        ...
        if ($json_de['status'] == 2) {
            //失败
            $json_de['errorCode'] . '/' . $json_de['errorMsg'];
        }
