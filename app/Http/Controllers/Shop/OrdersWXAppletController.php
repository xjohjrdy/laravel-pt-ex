...
class OrdersWXAppletController extends Controller
{
 ...
 //           }
            ...
            if ($request->type == 2) {
                ...
                if ($arrRequest['good_id'] == 1780) {
                    ...
                    if (empty($app_user) || $app_user->groupid < 23) {
                        return $this->getInfoResponse('4005', '暂无权限，该商品需要VIP权限购买！');
                    }
                }
                ...
                $res = $order->generateOrder($arrRequest['app_id'], [0 => ['app_id' => $arrRequest['app_id'], 'good_id' => $arrRequest['good_id'], 'shop_id' => $arrRequest['shop_id'], 'desc' => $arrRequest['desc'], 'number' => $arrRequest['number']]], 1);
            }
    ...
    public function show($id, Request $request, Order $order, ShopOrders $shopOrders, ShopOrdersOne $shopOrdersOne, ShopGoods $shopGoods, AdUserInfo $adUserInfo, UserAccount $userAccount, ShopAddress $shopAddress)
    {
        try {
            ...
            return $this->getResponse(['address' => $address, 'order_id' => $id, 'all_express' => $all_express, 'all_price' => $all_price, 'account_ptb' => $userAccount->extcredits4, 'deduct_ptb' => $deduct_ptb, 'order_detail' => $res_order_one]);
        } catch (\Exception $e) {
            if (!empty($e->getCode())) {
                throw new ApiException($e->getMessage(), $e->getCode());
            }
