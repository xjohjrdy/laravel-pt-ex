...
class OtherElemeMaidOld extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:OtherElemeMaidOld';
     ...
     * @var string
     */
    protected $description = 'Command 饿了么第三方管理费假库操作';
    ...
    public function __construct()
    {
        ...
        $this->maidOldModel = new EleMaidOldOut();
        $this->managerMaidService = new Maid();
        $this->otherMaidOldModel = new ThreeEleMaidOld();
...
//        $this->info($command->toJson());
        ...
        $delete_list = DB::connection('app38_out')->select(
            "select * from lc_ele_maid_old t1
        ...
        while (1) {
            try {
                ...
                foreach ($maid_list as $maid_item) {
                    ...
                    $taobaoOrderModel = new EleEnterOrderOut();
                    ...
                    $count_partner = 0;
                    try{
                        $this->managerMaidService->maid($order_sn, $commission, $tmp_next_id, $app_id, 6);
                    } catch (\Throwable $exception){
                    ...
                    for ($i = 1; $i < 50; $i++) {
                        ...
                        $has1 = !ThreeEleMaidOld::where(['trade_id' => (string)$order_sn, 'app_id' => (string)$p_pt_id])->exists();
                        $has2 = !EleMaidOldOut::where(['trade_id' => (string)$order_sn, 'app_id' => (string)$p_pt_id])->exists();
                        if ($has1 && $has2) {
    ...
    {
        Storage::disk('local')->append('callback_document/OtherMaid/ele.txt', var_export($msg, true));
    }
