...
use App\Entitys\OtherOut\MtMaidOldOut;
use App\Services\WuHang\Maid;
use Illuminate\Console\Command;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Storage;

class OtherMtMaidOld extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:OtherMtMaidOld';
     ...
     * @var string
     */
    protected $description = 'Command 美团第三方管理费假库操作';
    ...
    public function __construct()
    {
        ...
        $this->maidOldModel = new MtMaidOldOut();
        $this->otherMaidOldModel = new MtMaidOldOther();
...
//        $this->info($command->toJson());
        ...
        while (1) {
            try {
                ...
                foreach ($maid_list as $maid_item) {
                    ...
                    $taobaoOrderModel = new MtEnterOrderOut();
                    ...
                    $commission = $datum->balance_amount; #佣金金额(元)
                    $order_sn = $datum->unique_item_id;                 #订单号
                    $parent_info = AdUserInfoOut::where('pt_id', $app_id)->first(['groupid', 'pt_pid', 'pt_id']);
                    $tmp_next_id = $parent_info['pt_pid'];
                    $count_partner = 0;
                    try{
                        $this->managerMaidService->maid($order_sn, $commission, $tmp_next_id, $app_id, 5);
                    } catch (\Throwable $exception){
                    ...
                    for ($i = 1; $i < 50; $i++) {
                        ...
                        $has1 = !MtMaidOldOther::where(['trade_id' => (string)$order_sn, 'app_id' => (string)$p_pt_id])->exists();
                        $has2 = !MtMaidOldOut::where(['trade_id' => (string)$order_sn, 'app_id' => (string)$p_pt_id])->exists();
                        if ($has1 && $has2) {
                            MtMaidOldOther::create([
                                'father_id' => $app_id,
                                'order_enter_id' => 0,
    ...
    {
        Storage::disk('local')->append('callback_document/OtherMaid/mt.txt', var_export($msg, true));
    }
