 ...
 */
class SyncPddOrders2 extends Command
{
     ...
     * @var string
     */
    protected $signature = 'command:SyncPddOrders2';
    ...
    public function handle(PddEnterOrders $pddEnterOrders)
    {
        ...
        $day = 86400;
        $start_time = strtotime("2019-10-02 00:00:00");
        for ($i = 1; $i < 64; $i++) {
            $stime = $start_time + ($day * ($i - 1));
            $etime = $start_time + ($day * $i);
            $page = 1;
            $page_size = 20;
            $this->info("开始同步：" . date("Y-m-d H:i:s", $stime) . '--' . date("Y-m-d H:i:s", $etime));
            while (true) {
                $this->info("第：" . $page . '页' . ' 每页数量：' . $page_size);
                $response = json_decode($pddService->getOrder($stime, $etime, $page, $page_size), true);
                $data = @$response['data'];
