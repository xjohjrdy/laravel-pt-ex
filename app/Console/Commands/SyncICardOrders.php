 ...
 */
class SyncICardOrders extends Command
{
    protected $userAccount;
    protected $userModel;
    protected $cardMaidModel;
    protected $userMoneyService;
    protected $otherCountService;
     ...
     * @var string
     */
    protected $signature = 'command:SyncICardOrders';
    ...
    public function __construct()
    {
        ...
        $this->otherCountService = new OtherCountService();
    ...
    public function getOrdersList()
    {
        ...
        $star_date = date('Y-m-d\TH:m:s\.000+0800', strtotime('-3 day 0 minute'));
        $end_date = date('Y-m-d\TH:m:s\.000+0800');
        while (1) {
            $this->info('开始查询:' . $star_date . '-' . $end_date . ' pageNo:' . $page_no . ' size：' . $page_size);
            $response = json_decode($kaDuoFenServices->getOrderListBysupplier($dataFlag, $star_date, $end_date, $page_no, $page_size), true);
            if (empty($response)) {
