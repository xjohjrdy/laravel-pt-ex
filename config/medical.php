<?php
/**
 * User: Administrator
 * Date: 2019/7/25/025
 * Time: 9:35
 */

return [
    //医疗模块支付宝 PID
    'ali_pid' => '2088531728490041',

    //医疗模块支付宝配置
    'ali_config' => [
        'app_id' => '2019080266097538',
        'notify_url' => 'http://api.36qq.com/api/medical_make_XxX_ali_notify',
        'return_url' => 'http://api.36qq.com/api/medical_make_XxX_ali_notify',
        'ali_public_key' => '',
        'private_key' => '',
        'log' => [ // optional
            'file' => './logs/alipay.log',
            //'level' => 'debug'
        ],
        //'mode' => 'dev', // optional,设置此参数，将进入沙箱模式
    ],

    //医疗模块支付宝配置
    'ali_question_config' => [
        'app_id' => '2019080266097538',
        'notify_url' => 'http://api.36qq.com/api/medical_question_XxX_ali_notify',
        'return_url' => 'http://api.36qq.com/api/medical_question_XxX_ali_notify',
        'ali_public_key' => '',
        'private_key' => '',
        'log' => [ // optional
            'file' => './logs/alipay.log',
            //'level' => 'debug'
        ],
        //'mode' => 'dev', // optional,设置此参数，将进入沙箱模式
    ],

    //微信_医疗模块支付
    'we_config' => [
        'appid' => 'wxd2d9077a3072b5db', // APP APPID
//        'app_id' => 'wxb3fxxxxxxxxxxx', // 公众号 APPID
//        'miniapp_id' => 'wxb3fxxxxxxxxxxx', // 小程序 APPID
        'mch_id' => '1521224461',
        'key' => 'wuhang1231wuhang7890wuhang886655',
        'notify_url' => 'http://api.36qq.com/api/medical_make_XxX_we_notify',
//        'cert_client' => './cert/apiclient_cert.pem', // optional，退款等情况时用到
//        'cert_key' => './cert/apiclient_key.pem',// optional，退款等情况时用到
        'log' => [ // optional
            'file' => './logs/wechat.log',
            'level' => 'info', // 建议生产环境等级调整为 info，开发环境为 debug
            'type' => 'single', // optional, 可选 daily.
            'max_file' => 30, // optional, 当 type 为 daily 时有效，默认 30 天
        ],
        'http' => [ // optional
            'timeout' => 5.0,
            'connect_timeout' => 5.0,
            // 更多配置项请参考 [Guzzle](https://guzzle-cn.readthedocs.io/zh_CN/latest/request-options.html)
        ],
        'mode' => 'normal', // optional, dev/hk;当为 `hk` 时，为香港 gateway。
    ],

    //微信_医疗模块支付
    'we_question_config' => [
        'appid' => 'wxd2d9077a3072b5db', // APP APPID
//        'app_id' => 'wxb3fxxxxxxxxxxx', // 公众号 APPID
//        'miniapp_id' => 'wxb3fxxxxxxxxxxx', // 小程序 APPID
        'mch_id' => '1521224461',
        'key' => 'wuhang1231wuhang7890wuhang886655',
        'notify_url' => 'http://api.36qq.com/api/medical_question_XxX_we_notify',
//        'cert_client' => './cert/apiclient_cert.pem', // optional，退款等情况时用到
//        'cert_key' => './cert/apiclient_key.pem',// optional，退款等情况时用到
        'log' => [ // optional
            'file' => './logs/wechat.log',
            'level' => 'info', // 建议生产环境等级调整为 info，开发环境为 debug
            'type' => 'single', // optional, 可选 daily.
            'max_file' => 30, // optional, 当 type 为 daily 时有效，默认 30 天
        ],
        'http' => [ // optional
            'timeout' => 5.0,
            'connect_timeout' => 5.0,
            // 更多配置项请参考 [Guzzle](https://guzzle-cn.readthedocs.io/zh_CN/latest/request-options.html)
        ],
        'mode' => 'normal', // optional, dev/hk;当为 `hk` 时，为香港 gateway。
    ],

];